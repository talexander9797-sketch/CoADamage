...
local function PrintAnalysis(id)
    ...
    for _, definition in ipairs(CoA:GetStatDefinitions()) do
        print(string.format(
            "  %s: %s",
            definition.label,
            FormatRange(analysis.ranges[definition.key])
        ))
    end
    ...
    for _, definition in ipairs(CoA:GetStatDefinitions()) do
        local estimate = CoA:EstimateSingleStatScaling(
            analysis,
            definition.key
        )
...
local function PrintHelp()
    ...
    print("/coa queue - show queued cast snapshots")
...
SlashCmdList.COADAMAGE = function(msg)
    ...
    elseif command == "experiment" then
        ...
        if not experiment then
            ...
            print(string.format(
                "|cff33ff99Current experiment #%d|r",
                experiment.id
            ))

            print("  Reason: " .. tostring(
                experiment.reason or "Unknown"
            ))

            print("  Observations: " .. tostring(
                #(experiment.observations or {})
            ))
        end

    elseif command == "queue" then
        if type(CoA.GetQueueSize) == "function" then
            print(
                "|cff33ff99CoADamage:|r Queued snapshots: "
                .. CoA:GetQueueSize()
            )
        else
            print("|cffff3333CoADamage: CastQueue.lua did not load.|r")
        end
    ...
    elseif command == "debug" then
        ...
        print(
            "|cff33ff99CoADamage debug:|r "
            .. (CoA.debug and "ON" or "OFF")
        )
    ...
    elseif command == "clear" then
        ...
        if CoA.Experiment then
            CoA.Experiment.current = nil
        end

        if CoA.CastQueue and CoA.CastQueue.queue then
            CoA.CastQueue.queue = {}
        end

        print("|cff33ff99CoADamage:|r recorded spell, experiment, and queue data cleared.")
